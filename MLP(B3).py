...
while True:
    ...
    if option == 1:
        ...
        learning_rate = str(input("Please pick the learning rate SCHEDULE you want the algorith to use(usually constant): "))
        learning_rate_init = float(input("Please pick the learning rate init of the algorith: "))
        ...
        optimizer =  str(input("Please pick the optimizer you want the algorith to use: "))
        min_mse = float(input("Please pick the minimum MSE you want the algorith to stop at: "))
        momentum = 0
        if optimizer == 'sgd':
            momentum = float(input("Please pick the momentum you want the algorith to use: "))
            clf = MLPClassifier(hidden_layer_sizes=(hls,), activation=act, solver=optimizer, max_iter=epohs, random_state=0, learning_rate_init=learning_rate_init, learning_rate=learning_rate, momentum = momentum)
        else:
            clf = MLPClassifier(hidden_layer_sizes=(hls,), activation=act, solver=optimizer, max_iter=epohs, random_state=0, learning_rate_init=learning_rate_init, learning_rate=learning_rate)
    ...
    def plot(epohs, clf, ax1, ax2, ax3, ax4, fig, Xtest, Ltest, Xtrain, Ltrain, min_mse, callback=None):         
        ...
        def update(epohs):
            ...
            # x2 = Xtrain.max()
            # for i in range(hls):
            #     Yline = -((clf.coefs_[0][0,i] / clf.coefs_[0][1,i])*x1 + clf.intercepts_[0][i] ) 
            #     Ysus = -((clf.coefs_[0][0,i] / clf.coefs_[0][1,i])*x2 + clf.intercepts_[0][i] ) 
            #     plot2[i] = ax2.plot([x1, x2], [Yline, Ysus])[0]
            ...
            line, = ax4.plot([], [])
            mse.append(np.mean(np.mean((Ltrain - clf.predict(Xtrain))**2, axis=-1)))
            ...
            return scatter2_1, scatter2_2, contour2_1.collections[0], scatter3_1, scatter3_2, line, title2, title3, title4 #,*plot2
    ...
    if clf.solver == 'lbfgs':
        messagebox.showinfo(title="LBFGS detected", message=f'Unfortunately the "Limited memory Broyden–Fletcher–Goldfarb–Shanno" optimizer cannot be partially trained :(. Here are the results after the training is compelte')
        print('Unfortunately the "Limited memory Broyden–Fletcher–Goldfarb–Shanno" optimizer cannot be partially trained :(. Here are the results after the training is compelte')
